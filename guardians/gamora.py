...
def psf_rec_Vii(filename, err=None, fitting=True, covmodes=None, cov=None, RL=False):
    """
    ...
    if ((err is None) and (cov is None)):
        err = drax.get_err(filename, RL=RL)
        print("err before P shape", err.shape)
        err = P.dot(err)
    print("err after P shape", err.shape)
    ...
    if (cov is None):
        ...
        print("covmodes shape", covmodes.shape)
    else:
        covmodes = cov
    print("Done")
    Btt = f["Btt"][:]
    print("Btt shapes", Btt.shape)
